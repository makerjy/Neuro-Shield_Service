...
type ReportType = 'monthly' | 'quarterly' | 'bottleneck' | 'kpi' | 'intervention' | 'custom';
...
export function Reports({ centerId }: ReportsProps) {
  ...
  const templates: ReportTemplate[] = [
    {
      id: 'TPL-001',
      name: '월간 종합 운영 보고서',
      description: '관할 치매안심센터 전체 KPI, 이상징후, 개입 현황 종합',
      type: 'monthly',
      estimatedTime: '약 3분',
    },
    {
      id: 'TPL-002',
      name: '분기별 성과 보고서',
      description: '3개월 단위 추세 분석 및 목표 달성도 평가',
      type: 'quarterly',
      estimatedTime: '약 5분',
    },
    {
      id: 'TPL-003',
      name: '병목 센터 분석 보고서',
      description: '성과 저조 센터 심층 분석 및 개입 계획',
      type: 'bottleneck',
      estimatedTime: '약 4분',
    },
    {
      id: 'TPL-004',
      name: 'KPI 심층 분석 보고서',
      description: '특정 KPI에 대한 상세 분석 및 개선 방안',
      type: 'kpi',
      estimatedTime: '약 3분',
    },
    {
      id: 'TPL-005',
      name: '교육/인력 개입 실행 보고서',
      description: '진행 중인 개입 프로그램 현황 및 효과성 분석',
      type: 'intervention',
      estimatedTime: '약 2분',
    },
    {
      id: 'TPL-006',
      name: '맞춤형 보고서',
      description: '사용자 정의 항목 선택 및 보고서 생성',
      type: 'custom',
      estimatedTime: '약 5분',
    },
  ];
  ...
  const reports: Report[] = [
    {
      ...
      fileSize: '2.4 MB',
      pages: 28,
      lastModified: '2026-02-01 14:20',
    },
    {
      id: 'RPT-2026-01-003',
      title: '송파구·서초구 병목 분석 보고서',
      type: 'bottleneck',
      period: '2026.01.01 - 2026.01.31',
      generatedDate: '2026-01-25 14:15',
      generatedBy: '김행정',
      status: 'review',
      fileSize: '1.8 MB',
      pages: 15,
      district: '송파구, 서초구',
      uploadedFile: '송파서초_병목분석_수정본.pdf',
      lastModified: '2026-01-26 10:30',
    },
    {
      id: 'RPT-2025-Q4-001',
      ...
      generatedDate: '2026-01-05 10:00',
      generatedBy: '김행정',
      status: 'sent',
      fileSize: '4.1 MB',
      pages: 42,
      uploadedFile: '2025Q4_성과보고서_최종.pdf',
      lastModified: '2026-01-08 09:00',
    },
    {
      id: 'RPT-2026-02-002',
      title: '이탈률 심층 분석 보고서',
      ...
      generatedDate: '2026-02-03 11:20',
      generatedBy: '이철수',
      status: 'approved',
      fileSize: '1.2 MB',
      pages: 12,
      uploadedFile: '이탈률분석_검토완료.pdf',
      lastModified: '2026-02-03 16:45',
    },
    {
      id: 'RPT-2026-01-005',
      title: '강남구 센터 운영 보고서',
      ...
      generatedDate: '2026-01-02 09:00',
      generatedBy: '홍길동',
      status: 'generating',
      fileSize: '-',
      pages: 0,
  ...
  const getTypeBadgeColor = (type: ReportType) => {
    const colors = {
      ...
      quarterly: 'bg-purple-50 text-purple-700 border-purple-300',
      bottleneck: 'bg-red-50 text-red-700 border-red-300',
      kpi: 'bg-green-50 text-green-700 border-green-300',
      intervention: 'bg-yellow-50 text-yellow-700 border-yellow-300',
      custom: 'bg-gray-50 text-gray-700 border-gray-300',
    };
    return colors[type];
  };
  ...
  const handleCreateReport = () => {
    if (!selectedTemplate || !selectedPeriod) {
      alert('보고서 템플릿과 기간을 선택해주세요.');
      return;
    }

    console.log('[AUDIT] Report Draft Generation Started:', {
    ...
    alert('보고서 초안 생성이 시작되었습니다. 완료되면 다운로드하여 수정 후 업로드해주세요.');
  ...
  const handleDownload = (report: Report) => {
    console.log('[AUDIT] Report Downloaded:', {
      action: 'REPORT_DOWNLOAD',
      reportId: report.id,
      timestamp: new Date().toISOString(),
    });
    alert(`"${report.title}" 보고서 초안을 다운로드합니다.\n\n로컬에서 수정 후 "파일 업로드" 버튼으로 업로드해주세요.`);
  };
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && selectedReport) {
      console.log('[AUDIT] Report Uploaded:', {
  ...
  const handleSendConfirm = () => {
    if (selectedReport) {
      console.log('[AUDIT] Report Sent to Higher Organization:', {
        action: 'REPORT_SEND',
        reportId: selectedReport.id,
        timestamp: new Date().toISOString(),
      });

      alert(`"${selectedReport.title}" 보고서가 상급기관으로 전송되었습니다.\n전송 내역은 시스템에 기록됩니다.`);
  ...
  const filteredReports = reports.filter((report) => {
    ...
    if (selectedDistrict !== 'all' && report.district && !report.district.includes(selectedDistrict)) return false;
    return true;
  });
  ...
  return (
    <div className="h-full overflow-auto bg-white">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-gray-600 mt-1">
                보고서 초안 생성 → 로컬 수정 → 파일 업로드 → 검토 → 상급기관 전송
              </p>
            </div>
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-4">
        ...
        <section className="mb-6">
          <div className="bg-blue-50 border-2 border-blue-300 p-4">
            <h3 className="text-sm font-bold text-gray-900 mb-2">보고서 작성 프로세스</h3>
        ...
        <section>
            ...
            <div className="flex items-center gap-2">
              <Select value={filterType} onValueChange={setFilterType}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="bottleneck">병목 분석</SelectItem>
                  <SelectItem value="kpi">KPI 분석</SelectItem>
                  <SelectItem value="intervention">개입 실행</SelectItem>
                </SelectContent>
              </Select>
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="border-b-2 border-gray-900">
                ...
                <th className="text-center py-2 px-2 text-xs font-bold text-gray-900 w-24">생성자</th>
          ...
          <div className="mt-2 bg-gray-50 border border-gray-200 p-2 text-xs text-gray-600">
            <strong>보관 기간:</strong> 5년 | 
            <strong className="ml-2">접근 권한:</strong> 광역센터장, 행정담당자 | 
            <strong className="ml-2">백업:</strong> 매일 자동 | 
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="max-w-[700px]">
          <DialogHeader>
            ...
            <DialogDescription>
              시스템이 템플릿 기반 초안을 생성합니다. 다운로드 후 수정하세요.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="py-4 space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-2">
                ...
                >
                  <div className="font-semibold text-gray-900">2026.01</div>
                </button>
                <button
                ...
                >
                  <div className="font-semibold text-gray-900">2025.12</div>
                </button>
                <button
                  onClick={() => setSelectedPeriod('2025-Q4')}
                  className={`border-2 p-2 text-center transition-all text-xs ${
                ...
                >
                  <div className="font-semibold text-gray-900">2025 Q4</div>
                </button>
                <button
          ...
          <div className="py-4 space-y-4">
            <div className="bg-yellow-50 border border-yellow-300 p-4 text-xs">
              ...
              <ul className="list-disc ml-4 mt-2 space-y-1 text-gray-700">
                ...
                <li>지원 포맷: PDF, DOCX, HWP</li>
                <li>파일 크기: 최대 50MB</li>
                <li>업로드 후 상태가 "검토중"으로 변경됩니다</li>
              </ul>
            </div>
            ...
            <div className="border-2 border-dashed border-gray-300 p-8 text-center">
              ...
              <p className="text-xs text-gray-500">PDF, DOCX, HWP 형식 지원</p>
            </div>
          </div>
      ...
      <Dialog open={sendDialogOpen} onOpenChange={setSendDialogOpen}>
        <DialogContent className="max-w-[600px]">
          <DialogHeader>
            <DialogTitle>상급기관 전송</DialogTitle>
            <DialogDescription>
              {selectedReport?.title}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="py-4 space-y-4">
            <div className="bg-blue-50 border border-blue-300 p-4 text-xs">
              ...
              <ul className="mt-2 space-y-1 text-gray-700">
                <li>전송 대상: 보건복지부 치매정책과</li>
                <li>전송 방법: 행정안전망 (공문 자동 생성)</li>
            ...
            <div className="bg-yellow-50 border border-yellow-300 p-3 text-xs text-gray-700">
              <strong>확인사항:</strong> 전송 후에는 수정이 불가능합니다. 보고서 내용을 최종 확인하셨습니까?
            </div>
          </div>
