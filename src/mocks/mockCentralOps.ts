...
export async function fetchCentralRegions(
  ...
  const rows: RegionComparisonRow[] = REGION_LIST.map(r => {
    ...
    return {
      regionCode: r.code,
      regionName: r.name,
      signalQuality: Number(_sv(`${rs}-sq`, 80, 98).toFixed(1)),
      policyImpact: Number(_sv(`${rs}-pi`, 10, 45).toFixed(1)),
      bottleneckRisk: Number(_sv(`${rs}-br`, 15, 55).toFixed(1)),
      dataReadiness: Number(_sv(`${rs}-dr`, 78, 99).toFixed(1)),
      governanceSafety: Number(_sv(`${rs}-gs`, 85, 99).toFixed(1)),
